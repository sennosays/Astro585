...
function generate_number_of_planets_sample(eta, N::Integer)
  ...
  idx_w_planets = rand(N) .<= eta                        # single probability of having exactly one planet #.<= bitwise lessthan eq
...
#dist Returns a tuple of lists of orbital periods in days for planets, star masses and star radii for planet host stars
...
function generate_planet_sample(eta::Real, shape::Real, scale::Real, num_stars::Integer; minP = 0.0, maxP = Inf )
  ...
  # !! why are the star properties generated if they will eventaully be discarded? 
  ...
  idx_w_planets = find(num_planets_all)			       # Return a vector of the linear indexes of the non-zero elements
...
for k in 1:length(shapes)
    for j in 1:length(scales)
    	dist[:,j,k]= pmap(alpha->evaluate_model(stats_obs, alpha, shapes[j], scales[k], num_stars; minP=minP, maxP=maxP, num_evals=num_evals ),etas); 
    end # for j
end # for k
dist
end # function
